...
class KDTreeIndex(BaseIndex):
    ...
    def __init__(self, distance_metric: Callable[[np.ndarray, np.ndarray], float], dims: int) -> None:
    ...
    def search(self, query: np.ndarray, k: int = 5) -> list[SearchResult]:
        ...
        # Unpack the heap and flip the negative distances back to 
        results = [SearchResult(distance=-neg_dist, item=item) for neg_dist, _, item in heap]

        # Sort from closest (lowest distance) to furthest
        results.sort(key=lambda x: x.distance)
        return results

    def _knn_search(self, node: KDNode | None, query: np.ndarray, k: int, depth: int, heap: list[tuple[float, int, VectorItem]]) -> None:
        if node is None:
            return
    ...
    def save(self, filepath: str) -> None:
        ...
        with open(filepath, "wb") as f:
            pickle.dump(self._active_items, f)
        logger.info(f"KD-Tree Index successfully saved to {filepath} ({len(self._active_items)} items).")

    def load(self, filepath: str) -> None:
        """Loads items from disk and rebuilds the spatial tree."""
        if not os.path.exists(filepath):
            logger.warning(f"Index file {filepath} not found. Starting with a fresh database.")
            return
            
        with open(filepath, "rb") as f:
            self._active_items = pickle.load(f)
            
        # Reconstruct the physical tree structure from the loaded items
        self._rebuild()
        logger.info(f"KD-Tree Index successfully loaded and rebuilt from {filepath} ({len(self._active_items)} items).")